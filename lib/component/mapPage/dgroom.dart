...
class Dgroom extends StatefulWidget {
  final radius;
  Dgroom({Key? key, required this.radius}) : super(key: key);
  @override
  _DgroomState createState() => _DgroomState();
}

class _DgroomState extends State<Dgroom> {
  double? lat, lng;
  String? useridG;
  int? counter;
  String? case1 = "";
  ...
  Query<Map<String, dynamic>> reference =
      FirebaseFirestore.instance.collection("Maps").where("type", isEqualTo: "dgroom");
  late BitmapDescriptor customerMarker;

  getCustomMarker() async {
    customerMarker = await BitmapDescriptor.fromAssetImage(
        ImageConfiguration(devicePixelRatio: 123213), 'images/petgroom.png');
  }
