...
class Dvet extends StatefulWidget {
  final radius;
  Dvet({Key? key, required this.radius}) : super(key: key);
  @override
  _DvetState createState() => _DvetState();
}

class _DvetState extends State<Dvet> {
  double? lat, lng;
  String? useridG;
  int? counter;
  String? case1 = "";
  ...
  Query<Map<String, dynamic>> reference =
      FirebaseFirestore.instance.collection("Maps").where("type", isEqualTo: "dvet");
  late BitmapDescriptor customerMarker;

  getCustomMarker() async {
    customerMarker = await BitmapDescriptor.fromAssetImage(
        ImageConfiguration(devicePixelRatio: 123213), 'images/vet.png');
  }
